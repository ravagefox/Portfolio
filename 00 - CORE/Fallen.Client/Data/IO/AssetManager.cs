...
using System.IO;
using System.Linq;
using Client.Data.IO.ContentReaders;
using Engine.Assets;
...
namespace Client.Data.IO
{
    public sealed class AssetManager : IStreamContainer, ILoader, IDisposable
    {
        ...
        {
            if (this.archives.Digests.TryGetValue(digest, out var result))
            {
                return this.Load<T>(result);
            }
        ...
        public T Load<T>(string assetPath)
        {
            ...
            if (((IStreamContainer)this).CanOpenStream(tPath))
            {
                using (var stream = ((IStreamContainer)this).OpenStream(tPath))
                {
        ...
        private IEnumerable<MdxStream> GetMdxStreams()
        {
            var directoryInfo = new DirectoryInfo(this.Profile.RootDirectory);
            foreach (var fileInfo in directoryInfo.EnumerateFiles(MdxStream.Extension, SearchOption.AllDirectories))
            {
                yield return MdxStream.OpenRead(fileInfo.OpenRead());
            }
        }
        ...
        bool IStreamContainer.CanOpenStream(string assetPath)
        {
            ...
            {
                return this.archives.Any(s => s.Contains(assetPath));
            }
